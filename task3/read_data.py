...
def read_image_caption(path):
    # with open(path + '/train/train_caption.json', "r", encoding='utf-8') as f:
    with open(path + '/train/train_blip2.json', "r", encoding='utf-8') as f:
        train = json.loads(f.read())
        f.close()
    # with open(path + '/val/val_caption.json', "r", encoding='utf-8') as f:
    with open(path + '/val/val_blip2.json', "r", encoding='utf-8') as f:
        dev = json.loads(f.read())
        f.close()
    # with open(path + '/test/test_caption.json', "r", encoding='utf-8') as f:
    with open(path + '/test/test_blip2.json', "r", encoding='utf-8') as f:
...
def read_image(path):
    ...
    for f in files:
        ...
        ids.append(int(f.split('/')[-1].split('-')[2]))
        desc.append(f.split('/')[-1].split('-')[3])
...
def retrieve_data_for_verification(train_text, train_images):
    ...
    for claim_id in claim_ids:
        ...
        claim_object = (df['Claim'].values[0], text_evidences, image_evidences, df['cleaned_truthfulness'].values[0], claim_id, ruling_statement)
...
if __name__ == '__main__':
    ...
    val_claim = retrieve_data_for_verification(dev_text, train_image)
    train_claim = retrieve_data_for_verification(train_text, dev_image)
    test_claim = retrieve_data_for_verification(test_text, test_image)
