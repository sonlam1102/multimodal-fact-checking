...
from transformers import T5Tokenizer, T5ForConditionalGeneration, T5EncoderModel, OwlViTModel, AutoTokenizer, AutoModel, \
    LongformerTokenizer, LongformerModel, OwlViTProcessor, T5Model, EncoderDecoderModel, BartTokenizer, \
    BartForConditionalGeneration, LEDForConditionalGeneration, LEDTokenizer
...
def encode_one_sample(sample, device):
    claim = sample[0]
    ...
    # encoded_sample["image_evidence"] = len(image_evidence)
...
class ClaimExplanationDataset(torch.utils.data.Dataset):
    def __init__(self, claim_verification_data, device):
        self._data = claim_verification_data
        self._device = device

        self._encoded = []
        for d in self._data:
            self._encoded.append(encode_one_sample(d, self._device))
...
def train_model(train_data, device, batch_size, epoch=1, is_val=False, val_data=None, gen_model="led"):
    # if n_gpu:
    ...
    for e in range(epoch):
        # X, F, y, _ = make_batch(train_data, batch_size=batch_size, shuffle=True)
        gen_model.train()
        ...
        for i in range(len(X)):
            ...
            encoded_claim = tokenizer(main_seq, return_tensors="pt", truncation=True, padding="max_length",
                                    max_length=1000, return_attention_mask=True)
    ...
    # return best_encoder, best_decoder, loss_vals
    return best_model, tokenizer, loss_vals


def predict(test_data, device, gen_model, tokenizer, batch_size, n_gpu=None):
    # if n_gpu:
    #     device = torch.device('cuda:{}'.format(n_gpu) if torch.cuda.is_available() else 'cpu')
    # else:
    #     device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    X, F, y, z = make_batch(test_data, batch_size=batch_size)
    print('Predict.......')
...
if __name__ == "__main__":
    ...
    test_claim = ClaimExplanationDataset(val, device)
    ...
    with open('temp_dev.json', 'w', encoding='utf-8') as f:
        f.write(json.dumps(test_claim.to_list(), indent=4, cls=NpEncoder))
