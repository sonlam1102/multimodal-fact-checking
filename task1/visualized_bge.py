...
class Visualized_BGE(nn.Module):
    def __init__(self,
        ...
        if not from_pretrained:
            bge_config = AutoConfig.from_pretrained(model_name_bge, device_map='auto')
    ...
    def encode(self, image=None, text=None):
        ...
        if image is not None:
            ...
            if text is not None:
                text = self.tokenizer(text, return_tensors="pt", truncation=True, max_length=4000, padding=True)
    ...
    def encode_image(self, images):
        ...
        prompts = self.tokenizer(prompts, return_tensors="pt", padding=True, truncation=True, max_length=4000)        
